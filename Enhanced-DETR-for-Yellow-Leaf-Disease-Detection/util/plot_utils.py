...
def plot_mAP(logs, alias, ewm_col=0, log_name='log.txt', num_epoch=1):
    ...
    dfs = [pd.read_json(l,lines=True) for l in logs]
...
if __name__ == '__main__':
    files = list(Path('D:\\DEEP\\deformable_detr_efficientnet\\exps\\res-50_ddetr\\eval').glob('*.pth'))
    plot_precision_recall(files)
    plt.show()
    plot_logs(logs=Path('D:\\DEEP\\deformable_detr_efficientnet\\exps\\res-50_ddetr'),fields=('class_error', 'loss_bbox_unscaled', 'mAP'), ewm_col=0, log_name='log.txt')
    plt.show()
